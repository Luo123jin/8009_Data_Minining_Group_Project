...
* **XGBoost**
  使用二阶梯度、列采样和正则化加强泛化能力。实验中采用默认参数组合，如 `max_depth≈6`、`eta≈0.3`。
...
> 本研究基于以上模型流程，构建了 5 个基模型和 2 个集成模型。集成模型旨在探讨不同模型组合对恢复时间预测的影响，具体包括 Blending 与 Stacking，由于数据量较小，是否可以通过集成模型的方式充分发挥不同模型的优势，得到鲁棒性和稳定性更好的模型供后续研究使用。
---
...
## 6. 优化点
- 1. 考虑引入更多特征，如灾害发生时间、区域环境等，以提升模型对复杂场景的适应能力。
- 2. 考虑模型的计算效率和实际计算速度，确保在资源有限的情况下仍能获得较高的预测精度。
- 3. 考虑引入更多的灾害类型特征，如天气条件、社会经济指标等，以提升模型的泛化能力。
- 4. 考虑引入更多的时间序列特征，如前一个月的灾害发生次数、前一个月的经济损失等，以提升模型的预测能力。
- 5. 考虑引入更多的特征工程，如特征缩放、特征选择等，以提升模型的预测能力。
- 6. 考虑引入更多的模型参数调优，如学习率、树深度、正则化参数等，以提升模型的预测能力。目前的模型参数已在实验中进行了默认值设置，未来可考虑引入更多的参数调优。
- 7. 对字符型变量采取更加具有代表性的编码方式，如 One-Hot Encoding 或 Label Encoding，通过保证模型对字符型变量的有效利用，提升模型的预测能力。
### **总体解释**
